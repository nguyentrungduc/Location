...
- thụ động (passive) -> (CellID, WiFi MACID): location providers rất đặc biệt để nhận vị trí mà không thực sự bắt đầu sửa lỗi vị trí. location providers này có thể được sử dụng để nhận thụ động cập nhật vị trí khi các ứng dụng hoặc dịch vụ khác yêu cầu chúng mà không thực sự yêu cầu các vị trí đó. Location Provider sẽ trả lại các vị trí được tạo bởi các location provider khacs . Can permission ERIC.ACCESS_FINE_LOCATION, mặc dù nếu GPS không được bật, provider này chỉ có thể trả về các bản sửa lỗi thô. Đây là những gì Android gọi location provider này, tuy nhiên, các công nghệ cơ bản để làm cho công cụ này hoạt động được ánh xạ tới tập hợp cụ thể của phần cứng và khả năng cung cấp telco .
